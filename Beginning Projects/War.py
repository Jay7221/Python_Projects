...
class Game:
    ...
    def play_game(self):
        ...
        while len(cards) >= 2 and response != 'q':
            ...
            print('%s drew %s %s drew %s'%(self.player1.name, player1_card, self.player2.name, player2_card))
            if player1_card > player2_card:
                self.player1.wins += 1
                print('%s wins this round'%self.player1.name)
            else:
                self.player2.wins += 1
                print('%s wins this round.'%self.player2.name)
        print('The War is over. %s wins.'%self.winner(self.player1, self.player2))
